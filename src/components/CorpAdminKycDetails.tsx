...
export default function CorpAdminKycForm({ id }: { id: string }) {
  ...
  const { data: kycResponse, isLoading } = useQuery<
    ApiResponse<{
      verification: CorpKycData;
    }>
  >({
    queryKey: ["kyc-verification-corp", id],
    queryFn: async () => {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (_, { status }) => {
      toast.success(
        `Corporate KYC ${status === "APPROVED" ? "Approved" : "Rejected"} successfully`,
      );
    ...
    return (
      <div className="flex flex-col h-96 items-center justify-center space-y-4">
        ...
        <p className="text-gray-500 font-medium">
          Fetching corporate verification data...
        </p>
      </div>
    );
  }
    ...
    <div className="pb-20">
        ...
        <FormProvider {...methods}>
          <ThemeProvider className="space-y-4">
            <p className="text-sm text-gray-500">
              Please provide a reason for rejecting this corporate KYC
              submission.
            </p>
            <SimpleTextArea
              label="Rejection Reason"
              placeholder="e.g. CAC document is invalid or expired"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-4">
          ...
          />
          <InfoCard icon={Hash} label="RC Number" value={kycData?.rcNumber} />
          <InfoCard icon={ShieldCheck} label="Entity Type" value="Corporate" />
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <DocPreview url={kycData?.cacDocument} label="CAC Document" />
              <DocPreview url={kycData?.tinDocument} label="TIN Document" />
              <div className="sm:col-span-2">
                <DocPreview
                  url={kycData?.authorizedId}
            ...
            <div className="flex items-center justify-end gap-4 p-6 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
              <div className="mr-auto">
                <p className="font-bold text-gray-900">
                  Review Corporate Submission
                </p>
                <p className="text-sm text-gray-500">
                  Verify RC number and company documents.
                </p>
              </div>
              <Button
                variant="ghost"
