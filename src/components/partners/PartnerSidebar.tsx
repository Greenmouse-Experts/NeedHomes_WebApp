...
export function PartnerSidebar({ activePage, isSidebarOpen, setIsSidebarOpen }: PartnerSidebarProps) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={`fixed left-0 top-0 h-full w-64 bg-[#2A2A2A] text-white flex flex-col transform transition-transform duration-300 z-40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
                }`}>
                {/* Logo */}
                ...
                <nav className="flex-1 overflow-y-auto p-3 space-y-1">
                    ...
                    <Link
                        to="/partners"
                        onClick={handleLinkClick}
                        className={`flex items-center gap-2.5 p-2 rounded-lg text-sm transition-colors ${activePage === 'dashboard' || activePage === undefined
                            ? 'bg-[var(--color-orange)] text-white'
                            : 'hover:bg-gray-800 text-gray-400'
                            }`}
                        activeProps={{
