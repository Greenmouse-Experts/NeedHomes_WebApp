...
export const Route = createFileRoute("/properties/$propertyId/save-to-own/")({
...
function PropertyDetailPage() {
  ...
  const bankTransferMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const resp = await apiClient.post("/wallet/invest/initialize", {
        ...
        installmentFrequency: payload.savingsFrequency,
        installmentDuration: payload.savingsDuration,
      });
  ...
  const form = useForm({
    defaultValues: {
      installment: false,
      amount: 0,
      savingsFrequency: "MONTHLY" as "WEEKLY" | "MONTHLY",
      savingsDuration: 12 as 3 | 6 | 12,
    },
  });
  ...
  return (
    <PageLoader query={query}>
      {(data) => {
        ...
        const minimumInstallment = Math.ceil(totalPrice / savingsDuration);
        useEffect(() => {
          if (installOptions) {
                  ...
                  {!auth?.accessToken ? (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (auth?.user?.accountType === "INVESTOR") {
                          return navigate({
                            to: "/investors/properties/$propertyId/save-to-own/",
                            params: { propertyId },
                          });
                        }
                    ...
                    >
                      {paymentMethod === "BANK_TRANSFER"
                        ? `Pay via Bank Transfer ${formatCurrency(minimumInstallment)}`
                        : `Confirm & Pay ${formatCurrency(minimumInstallment)}`}
                    </Button>
                  )}
                </div>
              }
                      ...
                      {(["WEEKLY", "MONTHLY"] as const).map((freq) => (
                        ...
                        >
                          <input
                            ...
                            className="hidden"
                            value={freq}
                            {...form.register("savingsFrequency")}
                          />
